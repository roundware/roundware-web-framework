...
describe("SpeakerEngine - repeatLoopOnLoopPoint", () => {
  ...
  it("should throw an error if base track is null after getting from speakers list", () => {
    ...
    jest
      ...
      .mockReturnValue(baseTrack as any);
  ...
  it("should set exceededDuration to 0 when it is nearly zero", () => {
    ...
    const utilsModule = require("../utils");
    const isNearlyZeroSpy = jest
      .spyOn(utilsModule, "isNearlyZero")
  ...
  it("should configure playWithConfig correctly and add track finished listener when not continued", () => {
    ...
    expect(track.on).toHaveBeenCalledWith(
      "trackFinished",
      expect.any(Function)
    );
  });
  ...
  it("should skip loop point update when probability is 0", () => {
    ...
    jest
      .spyOn(SpeakerUtils, "shouldDoSomethingWithProbability")
...
describe("SpeakerEngine", () => {
  ...
  describe("play method", () => {
    ...
    it("should not throw when PREFETCH strategy and all speakers loaded", async () => {
      ...
      speakerEngine.speakers = [{ buffer: {} } as any, { buffer: {} } as any];
    ...
    describe("stop method", () => {
      it("should stop playback and clean up resources", async () => {
        ...
        expect(mockSpeaker1.clearListeners).toHaveBeenCalledWith(
          "trackFinished"
        );
        expect(mockSpeaker1.clearListeners).toHaveBeenCalledWith(
          "trackAborted"
        );
        expect(mockSpeaker2.clearListeners).toHaveBeenCalledWith(
          "trackFinished"
        );
        expect(mockSpeaker2.clearListeners).toHaveBeenCalledWith(
          "trackAborted"
    ...
    describe("updateParams method", () => {
      ...
      it("should update mixParams and handle PROGRESSIVE loading strategy", () => {
        ...
        jest
          .spyOn(require("@turf/point-to-polygon-distance"), "default")
          .mockImplementation((point: any, polygon: any) => {
...
describe("SpeakerEngine - group initialization", () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(SpeakerUtils, "getRootForSpeaker")
      .mockImplementation(
        (
...
describe("SpeakerEngine - onLocationUpdateProgressiveBasePlusMaxNRandom", () => {
  ...
  it("should update base track when a new base track is selected", () => {
    ...
    jest
      .spyOn(speakerEngine, "latestBaseTrack", "get")
      .mockReturnValue(mockBaseTrack);
    ...
    expect(emitSpy).toHaveBeenCalledWith(
      ...
      expect.arrayContaining([mockBaseTrack.data.id])
    );
  });
  ...
  it("should load buffer and play when new base track is selected but buffer not loaded", () => {
    ...
    expect(mockBaseTrack.on).toHaveBeenCalledWith(
      "loaded",
      expect.any(Function)
    );
    ...
    const loadedCallback = (mockBaseTrack.on as jest.Mock).mock
      .calls[0][1] as () => void;
    loadedCallback();
  ...
  it("should stop other tracks when new base track is selected", () => {
    ...
    expect(mockSpeakerTrack.clearListeners).toHaveBeenCalledWith(
      "trackFinished"
    );
    expect(mockSpeakerTrack.clearListeners).toHaveBeenCalledWith(
      "trackAborted"
  ...
  it("should update volumes when base track remains the same", () => {
    ...
    jest
      .spyOn(speakerEngine, "latestBaseTrack", "get")
      .mockReturnValue(mockSpeakerTrack);
  ...
  it("should not update tracks when not playing", () => {
    ...
    const calculateVolumesSpy = jest.spyOn(
      speakerEngine,
  ...
  it("should handle slot consideration skip correctly", () => {
    ...
    const shouldDoSomethingSpy = jest
      .spyOn(SpeakerUtils, "shouldDoSomethingWithProbability")
    ...
    expect(shouldDoSomethingSpy).toHaveBeenCalledWith(
      0.5,
      "slot consideration"
    );
  ...
  it("should handle null tracks in playingTracks when updating volumes", () => {
    ...
    jest
      .spyOn(speakerEngine, "getSpeakerTrackById")
      .mockImplementation((id: number): SpeakerTrack => {
        if (id === mockSpeakerTrack.data.id)
          return mockSpeakerTrack as unknown as SpeakerTrack;
        if (id === mockBaseTrack.data.id)
          return mockBaseTrack as unknown as SpeakerTrack;
...
describe("SpeakerEngine - onLoopPoint", () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(speakerEngine, "playAsBaseTrack")
      .mockImplementation((track: SpeakerTrack, isCurrent: boolean) => {
    ...
    jest
      .spyOn(speakerEngine, "clearEndListeners")
  ...
  it("should handle base track change", () => {
    ...
    expect(emitSpy).toHaveBeenCalledWith("playingTracksUpdated", [
      mockBaseTrack.data.id,
    ]);
  });
  ...
  it("should stop playing tracks with volume below minVolume", () => {
    ...
    jest
      ...
      .mockImplementation((id: number): SpeakerTrack => {
        if (id === mockSpeakerTrack.data.id)
          return mockSpeakerTrack as unknown as SpeakerTrack;
        if (id === lowVolumeTrack.data.id)
          return lowVolumeTrack as unknown as SpeakerTrack;
        throw new Error("Track not found");
      });
...
describe("SpeakerEngine - playAsBaseTrack", () => {
  ...
  it("should add track finished listener when not continued", () => {
    ...
    expect(mockSpeakerTrack.on).toHaveBeenCalledWith(
      "trackFinished",
      expect.any(Function)
    );
  });
...
describe("SpeakerEngine - fadeOutLoopFromLoopPoint", () => {
  ...
  it("should handle different loop configurations", () => {
    ...
    testCases.forEach(({ duration, times }) => {
      ...
      jest
        .spyOn(SpeakerUtils, "findRemainingTime")
        .mockReturnValue((duration * times) / 2);
...
describe("SpeakerEngine - Constructor", () => {
  ...
  beforeEach(() => {
    ...
    jest
      .spyOn(SpeakerUtils, "getRootForSpeaker")
      .mockImplementation((data) => data.id);
  });
  ...
  it("should initialize with prefetch strategy", () => {
    ...
    jest
      .spyOn(SpeakerUtils, "getLoadingStrategy")
      .mockReturnValue(LoadingStrategy.PREFETCH);

    // Create a spy on SpeakerTrack.prototype.loadBuffer
    const loadBufferSpy = jest.spyOn(
      SpeakerTrack.prototype,
      "loadBuffer"
    ) as jest.Mock;
  ...
  it("should initialize with progressive strategy", () => {
    ...
    jest
      .spyOn(SpeakerUtils, "getLoadingStrategy")
      .mockReturnValue(LoadingStrategy.PROGRESSIVE);
...
describe("SpeakerEngine - updateNonBaseTracks", () => {
  ...
  it("should handle buffer loading for new speaker", () => {
    ...
    // Simulate buffer loaded event
    const loadedCallback = (newSpeaker.on as jest.Mock).mock
      .calls[0][1] as () => void;
    const now = mockAudioContext.currentTime;
    loadedCallback();
